...
/* Représente une salle avec un ajout d'itentifiant en int pour faciliter la gestion des dvar*/
tuple Salle{
...
/* Représente les indemnités de la session */
tuple Indemnite {
...
{string} listPersonnelsIndemnites[0..30];// trente étant le nombre moyen d'intervenant 
...
execute{	
	...
	******************************************************************************/	
	...
	coutMaximal=0;// initialisation de coutMaximal
	for(s in sessions){
		for (i in intervenantsDeSession[s.idSession]){
...
// entier qui recuperera la somme des indemnités. c'est la valeur que l'on veut minimiser
...
//on optimise le couttotal grace à minimize
minimize 
	couttotal;
subject to {
